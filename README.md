# 👋 Hello, I'm Sherif
CS undergrad | Software engineer | aspiring solutions architect

I build things for the cloud — from scalable infrastructure to intelligent applications.  
I enjoy designing systems that blend **automation, AI, and clean architecture**.

---

###  What I Do
-  Cloud Infrastructure – AWS | Terraform | Docker | ECS | Lambda  
-  Backend Development – Node.js | Express | Python | Java | Spring Boot  
-  AI-Driven Apps – Amazon Bedrock | Claude | Textract | Polly  

---

### Hobbies
- Building serverless tools
- Drawing beautiful landscapes
- watching animated movies and shows

---
### 🛠️ Selected Projects
- **Cashflow AI** – AI-powered finance manager using AWS Bedrock & Claude  
- **Audio Notes** – Serverless app converting handwritten notes to audio  
- **Automated WordPress Deployment** – Scalable ECS-based setup using Terraform  

Explore my work → [github.com/sherifsani](https://github.com/sherifsani)

---

### 📫 Get in Touch
 [LinkedIn](https://linkedin.com/in/sherif-sani)  
 [Twitter](https://twitter.com/sherifsani)  
 [sanisherif838@gmail.com](mailto:sanisherif838@gmail.com)


