# 👋 Hello, I'm Sherif

**CS undergrad | Software engineer | aspiring solutions architect**

I build things for the cloud — from scalable infrastructure to intelligent applications.  
I enjoy designing systems that blend **automation, AI, and clean architecture**.

<br>

## What I Do
.
**Cloud Infrastructure**  
AWS · Terraform · Docker · CI/CD · Lambda

**Backend Development**  
Node.js · Express · Python · Java · Spring Boot

**Frontend dev**  
React · Nextjs · Tailwind · Typescript

<br>

## Selected Projects

**[Cashflow AI](https://github.com/sherifsani/cashflow-ai)**  
AI-powered finance manager using AWS Bedrock & Claude

**[Audio Notes](https://github.com/sherifsani/audio-notes)**  
Serverless app converting handwritten notes to audio

**[Automated WordPress Deployment](https://github.com/sherifsani/wordpress-terraform)**  
Scalable ECS-based setup using Terraform

<br>

## Hobbies

Building serverless tools · Drawing beautiful landscapes · Watching animated movies and shows · watching/participating in sports

<br>

## Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://linkedin.com/in/sherif-sani)
[![Twitter](https://img.shields.io/badge/Twitter-1DA1F2?style=flat&logo=twitter&logoColor=white)](https://twitter.com/sherifsani)
[![Email](https://img.shields.io/badge/Email-EA4335?style=flat&logo=gmail&logoColor=white)](mailto:sanisherif838@gmail.com)
